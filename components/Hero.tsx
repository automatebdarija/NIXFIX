  ...
  return (
    <section className="relative w-full min-h-[90vh] flex flex-col items-center justify-center overflow-hidden pt-32 pb-16 lg:pt-40 bg-black">
      ...
      <div className="absolute inset-0 z-0 opacity-40 mix-blend-overlay">
        <img 
          src="https://i.ibb.co/1Jf7BwTb/HD-wallpaper-black-banner-1-1.jpg"
        ...
        <div className="animate-in fade-in duration-1000 delay-500">
            <div className="flex flex-col md:flex-row items-center gap-6 p-4 rounded-2xl glass-panel">
                <div className="flex -space-x-4 rtl:space-x-reverse">
                    {[1,2,3,4].map((i) => (
                        <div key={i} className={`w-10 h-10 rounded-full border-2 border-black flex items-center justify-center font-bold text-xs text-white ${
                            i===1 ? 'bg-purple-600' : i===2 ? 'bg-blue-600' : i===3 ? 'bg-green-600' : 'bg-brand-gold text-black'
                        }`}>
                            {i===4 ? 'A' : 'U'}
                        </div>
                    ))}
